...
CREATE TABLE IF NOT EXISTS chat_messages (
    ...
    sender_staff_id    UUID NOT NULL REFERENCES staff(staff_id),
...
CREATE INDEX IF NOT EXISTS idx_chat_messages_participants
    ON chat_messages(sender_staff_id, recipient_staff_id, sent_at);
...
-- =====================================================================
-- SECTION 8: HIPAA — FIELD ENCRYPTION EXAMPLE
-- =====================================================================
...
-- Run after hospital db schema.sql.
-- Generate password hashes with: node -e "console.log(require('bcryptjs').hashSync('change-me', 12))"

CREATE TABLE IF NOT EXISTS app_users (
    user_id       UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    staff_id      UUID UNIQUE REFERENCES staff(staff_id),
    email         VARCHAR(150) UNIQUE NOT NULL,
...
-- Staff accounts are created through the registration screen, which generates
-- the work email and staff UUID automatically.

-- Run after hospital db schema.sql to support inventory and Ghana NHIS claims.
INSERT INTO roles (role_name, description)
VALUES
    ('nurse', 'Patient details and nursing operations'),
    ('doctor', 'Full clinical and administrative access'),
    ('pharmacy', 'Doctor-issued prescription dispensing'),
    ('lab', 'Patient records and laboratory operations')
ON CONFLICT (role_name) DO NOTHING;

CREATE TABLE IF NOT EXISTS inventory_items (
    item_id       UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    item_name     VARCHAR(150) NOT NULL,
    category      VARCHAR(50) NOT NULL, -- medicine, equipment, consumable
...
INSERT INTO insurance_providers (name, contact_phone, contact_email)
SELECT 'Ghana National Health Insurance Scheme (NHIS)', '0302 611 611', 'info@nhia.gov.gh'
WHERE NOT EXISTS (SELECT 1 FROM insurance_providers WHERE name = 'Ghana National Health Insurance Scheme (NHIS)');
